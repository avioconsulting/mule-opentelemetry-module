...
public class BatchTransaction extends AbstractTransaction {
  ...
  private final ConcurrentHashMap<String, AtomicReference<FlowSpan>> stepSpans = new ConcurrentHashMap<>();
   ...
   *            {@link TraceComponent}
   * @return A newly created or an existing {@link FlowSpan} for this step
   *         location
   */
  private FlowSpan addOrGetContainerSpan(final String location, String stepName, TraceComponent processorTrace) {
    AtomicReference<FlowSpan> containerSpanRef = stepSpans.computeIfAbsent(location,
    ...
    FlowSpan containerSpan = containerSpanRef.get();
    if (containerSpan == null) {
      ...
      stepTraceComponent.getTags().putAll(processorTrace.getTags());
      if (stepName != null) {
      ...
      SpanBuilder spanBuilder = spanBuilderFunction.apply(name)
          .setParent(rootSpan.storeInContext(Context.current()));
      FlowSpan newContainerSpan = new FlowSpan(location, spanBuilder.startSpan(),
          processorTrace.getTransactionId());
      ProcessorSpan stepProcessorSpan = new ProcessorSpan(newContainerSpan.getSpan(), location,
          stepTraceComponent.getTransactionId(),
          stepTraceComponent.getStartTime(), stepTraceComponent.getSpanName());
  ...
  private SpanMeta processContainerChild(String containerPath, TraceComponent traceComponent,
      ...
      FlowSpan stepSpan, ProcessorSpan processorSpan) {
    SpanMeta spanMeta;
    if (isBatchOnComplete(containerPath, componentLocator)) {
      ...
      if (traceComponent.getLocation().equalsIgnoreCase(containerPath + "/processors/0")) {
        ...
        SpanMeta recordSpanMeta = stepSpan.addChildFlow(recordTrace,
  ...
  public SpanMeta endProcessorSpan(TraceComponent traceComponent, Consumer<Span> spanUpdater, Instant endTime) {
    if (BATCH_STEP_TAG.equalsIgnoreCase(traceComponent.getName())
      ...
      if (spanName != null) {
        ...
        FlowSpan flowSpan = stepSpans.get(stepLocation).get();
        SpanMeta spanMeta = flowSpan.endProcessorSpan(traceComponent, spanUpdater, endTime);
        ...
        if (traceComponent.getLocation() != null
          ...
          ProcessorSpan aggrSpan = flowSpan.findSpan(traceComponent.contextScopedPath(aggregatorLocation));
  ...
  private ProcessorSpan endContainerSpan(TraceComponent traceComponent, ProcessorSpan processorSpan) {
    if (processorSpan == null)
      return null;
    FlowSpan stepSpan = stepSpans.get(processorSpan.getLocation()).get();
  ...
  public void addChildTransaction(TraceComponent traceComponent, SpanBuilder spanBuilder) {
    ...
    FlowSpan stepSpan = stepSpans.get(processorSpan.getLocation()).get();
    stepSpan.addChildFlow(traceComponent, spanBuilder);
  }
  ...
  public TransactionMeta endChildTransaction(TraceComponent traceComponent, Consumer<Span> endSpan) {
    ProcessorSpan processorSpan = getStepProcessorSpan(traceComponent);
    FlowSpan stepSpan = stepSpans.get(processorSpan.getLocation()).get();
    return stepSpan.endChildFlow(traceComponent, endSpan);
  }

  @Override
  public ProcessorSpan findSpan(String location) {
    ProcessorSpan processorSpan = null;
    for (Map.Entry<String, AtomicReference<FlowSpan>> entry : stepSpans.entrySet()) {
