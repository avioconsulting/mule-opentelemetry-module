...
NOTE: When configuring the OTLP Exporter with *HTTP Protobuf protocol*, OpenTelemetry collector endpoint *must be set to the base endpoint* of OTEL collector. The module will build trace-specific endpoints `{collectorEndpoint}/traces` based on OpenTelemetry specification guidelines.
...
Extension uses a processor interceptor. OpenTelemetry's tracing context will be automatically added to a flow variable named **OTEL_TRACE_CONTEXT** as below -
