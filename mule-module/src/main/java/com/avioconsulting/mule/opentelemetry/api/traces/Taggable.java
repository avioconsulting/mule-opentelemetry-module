 ...
 * The interface is generic, allowing flexibility in defining the types of keys and values.
 *
...
public interface Taggable<K, V> {
   ...
   *
   * @param source the map containing tags to be added
   */
   ...
   *
   * @param target the map to which tags will be copied
   */
   ...
   *
   * @param target the taggable object to which tags will be copied
   */
  void copyTagsTo(Taggable<K, V> target);

  /**
   * Copies tags from this taggable object to another taggable object, but only for keys
   * that satisfy the provided predicate filter.
   *
   * @param target    the taggable object to which tags will be copied
   * @param keyFilter a predicate that determines which keys should be copied
   */
  ...
  /**
   * Checks if this taggable object contains a tag with the specified key and value.
   *
   * @param key   the key to check for
   * @param value the value to check for
   * @return true if a tag with the specified key and value exists, false otherwise
   */
  boolean containsTag(K key, V value);

  /**
   * Performs the given action for each tag entry in this taggable object.
   * The behavior is unspecified if the taggable object is modified during iteration.
   *
   * @param entry the action to be performed for each tag entry
   */
