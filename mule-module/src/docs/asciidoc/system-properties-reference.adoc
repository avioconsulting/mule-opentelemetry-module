...
|mule.otel.periodic.logger.cache.size
|MULE_OTEL_PERIODIC_LOGGER_CACHE_SIZE
|Maximum number of unique log messages to cache for throttling.
|`10000`

|mule.otel.periodic.logger.interval.seconds
|MULE_OTEL_PERIODIC_LOGGER_INTERVAL_SECONDS
|The minimum time interval (in seconds) between identical log messages.
|`60`

|mule.otel.periodic.logger.cleanup.multiplier
|MULE_OTEL_PERIODIC_LOGGER_CLEANUP_MULTIPLIER
|Multiplier for the throttle interval to determine how often to clean up stale entries. For example, if interval is 60s and multiplier is 3, cleanup runs every 180s.
|`3`
...
|System property to enable or disable span generation for all processors. If set to true, the module creates spans for every processor in the flow. This can be overridden by the trace level configuration.
|`false`
